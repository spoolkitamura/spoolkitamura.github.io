...
//    this.canvas.style.position = 'absolute';
    ...
    // (touch)
    this.canvas.addEventListener('touchstart', this.touchDownHandler, false);
    this.canvas.addEventListener('touchend',   this.touchUpHandler,   false);
  ...
  touchDownHandler(e) {    // In here, 'this' represents 'canvas'
    if(e.cancelable) {
      e.preventDefault();  // デフォルトアクションをキャンセル
    }
    let touch     = e.changedTouches.item(0);
    let rectCnavs = this.getBoundingClientRect();
    let x         = Math.round(touch.clientX - rectCnavs.left);
    let y         = Math.round(touch.clientY - rectCnavs.top);
    //console.log("S", x, y);
    Event.setMousePosition(x, y);
    Event.setMouseState(0, true);    // LBUTTON
  }

  touchUpHandler(e) {      // In here, 'this' represents 'canvas'
    // デフォルトアクションをキャンセル
    if(e.cancelable) {
      e.preventDefault();  // デフォルトアクションをキャンセル
    }
    ...
    //console.log("E", x, y);
    Event.setMousePosition(x, y);
    Event.setMouseState(0, false);   // LBUTTON
  }

  touchMoveHandler(e) {    // In here, 'this' represents 'canvas'
    // デフォルトアクションをキャンセル
    if(e.cancelable) {
      e.preventDefault();  // デフォルトアクションをキャンセル
    }
